        ...
        try {
            SearchResponse<RestaurantDocument> response = client.search(s -> s
                            ...
                            .query(q -> q
                                    .multiMatch(m -> m
                                            ...
                                            .fields("name", "foodNames", "cuisineType")
                                    )
                            ),
                    RestaurantDocument.class
            );
            ...
            return response.hits().hits().stream()
                    .flatMap(hit -> {
                        ...
                        Stream<String> nameStream = doc.getName() != null ? Stream.of(doc.getName()) : Stream.empty();
                        Stream<String> cuisineStream = doc.getCuisineType() != null ? Stream.of(doc.getCuisineType()) : Stream.empty();
                        Stream<String> foodNamesStream = doc.getFoodNames() != null
                                ? doc.getFoodNames().stream().filter(Objects::nonNull)
                                : Stream.empty();

                        return Stream.concat(Stream.concat(nameStream, cuisineStream), foodNamesStream);
                    })
